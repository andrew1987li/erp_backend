...
define(["jquery", "jIAn", "datePicker", "datePicker_zh_tw"], function ($, jIAn) {
    ...
    var bindParam = function () {
        ...
        form.add_variable("AnnouncementType", $("#at").val());
        ...
        run = function (param) {
            ...
            $("#at").val(data.AnnouncementType.toString());

            $("#search").click(function () {
                bindParam();
                form.submit_form("post", "/Ammas/Cms/Announcements");
            });
            ...
            $("#add").click(function () {
                form.add_variable("Announcement.AnnouncementId", 0);
                bindParam();
                form.submit_form("post", "/Ammas/Cms/AnnouncementEdit");
            });

            $(":button[id*=\"edit_\"]").click(function () {
                var id = $(this).attr("id").split("_")[1];
                form.add_variable("Announcement.AnnouncementId", id);
                bindParam();
                form.submit_form("post", "/Ammas/Cms/AnnouncementEdit");
            });
            ...
            $(":button[id*=\"delete_\"]").click(function () {
                ...
                modal("confirm", "確定要刪除 ?", {
                    Click_OK: function (e) {
                        ...
                        $.when($.ajax({
                            type: "post",
                            url: "/Ammas/Cms/AjaxAnnouncementDelete",
                            data: { "Announcement.AnnouncementId": id }
                            ...
                            if (code > 0) {
                                var parameter = [
                                    "StartDate=" + $("#start").val(),
                                    "EndDate=" + $("#end").val(),
                                    "AnnouncementType=" + $("#at").val(),
                                    "KeyWord=" + $("#kw").val()
                                ].join("&");

                                form.add_variable("IsTransfer", true);
                                form.add_variable("Target", "/Ammas/Cms/Announcements");
