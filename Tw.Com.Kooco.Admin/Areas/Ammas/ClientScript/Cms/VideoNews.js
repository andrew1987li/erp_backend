...
define(["jquery", "jIAn", "datePicker", "datePicker_zh_tw"], function ($, jIAn) {
    ...
    var bindParam = function () {
        ...
        form.add_variable("VideoNewsType", $("#at").val());
        ...
        run = function (param) {
            ...
            $("#at").val(data.VideoNewsType.toString());

            $("#search").click(function () {
                bindParam();
                form.submit_form("post", "/Ammas/Cms/VideoNews");
            });
            ...
            $("#add").click(function () {
                form.add_variable("VideoNews.VideoNewsId", 0);
                bindParam();
                form.submit_form("post", "/Ammas/Cms/VideoNewsEdit");
            });

            $(":button[id*=\"edit_\"]").click(function () {
                var id = $(this).attr("id").split("_")[1];
                form.add_variable("VideoNews.VideoNewsId", id);
                bindParam();
                form.submit_form("post", "/Ammas/Cms/VideoNewsEdit");
            });
            ...
            $(":button[id*=\"delete_\"]").click(function () {
                ...
                modal("confirm", "確定要刪除 ?", {
                    Click_OK: function (e) {
                        ...
                        $.when($.ajax({
                            type: "post",
                            url: "/Ammas/Cms/AjaxVideoNewsDelete",
                            data: { "VideoNews.VideoNewsId": id }
                            ...
                            if (code > 0) {
                                var parameter = [
                                    "StartDate=" + $("#start").val(),
                                    "EndDate=" + $("#end").val(),
                                    "VideoNewsType=" + $("#at").val(),
                                    "KeyWord=" + $("#kw").val()
                                ].join("&");

                                form.add_variable("IsTransfer", true);
                                form.add_variable("Target", "/Ammas/Cms/VideoNews");
